...
//import io.hydrosphere.mist.{Mist, MistConfig, Repository, Specification, _}
...
private [mist] object InMapDbJobConfigurationRepository extends ConfigurationRepository {
  ...
  private lazy val db  =  DBMaker
    .fileDB("file.db") //TODO add conf MistConfig.Recovery.recoveryDbFileName
    .fileLockDisable
    .closeOnJvmShutdown
    .make
...
private[mist] object RecoveryJobRepository extends JobRepository {
  ...
  lazy val configurationRepository: ConfigurationRepository = InMemoryJobConfigurationRepository /*MistConfig.Recovery.recoveryTypeDb match {
...
*/ // TODO add config
  override def add(job: Job): Unit = {
    ...
    if(job.jobRunnerName == Constants.Actors.asyncJobRunnerName) {
      ...
      //Mist.recoveryActor ! JobStarted //TODO add recovery actor
    }
  }
  ...
  def removeFromRecovery(job: Job): Unit = {
    if(job.jobRunnerName == Constants.Actors.asyncJobRunnerName) {
      ...
      //Mist.recoveryActor ! JobCompleted //TODO add recovery actor
    }
  }
}